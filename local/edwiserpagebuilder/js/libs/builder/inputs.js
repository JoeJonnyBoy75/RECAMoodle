...
var SelectInput = $.extend({}, Input, {
	...
	updateOptions: function (options) {
		$.each(options, function (index, value) {
			// if ($('select', this.element).find(`option[value="${value.value}"]`).length > 0) {
			var option = (value.checked) ? ' selected': '';
			$('select', this.element).append(`<option value="${value.value}" ${option}>${value.text}</option>`);
			// }
		});
	},
...
var CssUnitInput = $.extend({}, Input, {
	...
	setValue: function (value) {
		...
		this.unit = this.unit.includes('px') ? 'px' : this.unit;
