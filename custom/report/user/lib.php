        ...
        if ($type == 'item' or $type == 'categoryitem' or $type == 'courseitem') {
            ...
            if (!$hide) {
                ...
                **/
                ...
                /// Name
		if (!empty($cm)) {
			$url_params = 'pass=cm&user=' . $this->user->id . '&course=' . $this->courseid . '&cm=' . $cm->id;
                	$data['itemname']['content'] = $fullname . ' <div class="warning-button warning-button-small"><a href="set_grade_student_scorm.php?' . $url_params . '" target="_blank">PASS USER</a></div>';
		} else {
