...
require('../config.php');
require_once($CFG->dirroot.'/custom/report/outline/locallib.php');
...
$PAGE->set_url('/custom/user_reset_report.php', array('id'=>$userid, 'course'=>$courseid, 'mode'=>$mode));
...
$PAGE->set_url('/custom/user_reset_report.php', array('id'=>$user->id, 'course'=>$course->id, 'mode'=>$mode));
...
$navigationnode = array(
        'url' => new moodle_url('/custom/user_reset_report.php', array('id' => $user->id, 'course' => $course->id, 'mode' => $mode))
    );
if ($mode === 'complete') {
...
// (gav) 20190821 Added reset section.
echo <<<STYLEEND
<style type="text/css">
.warning-container {
    margin: 15px;
...
STYLEEND;
echo "<div class=\"warning-container\"><div class=\"warning-button warning-button-large\"><a href=\"/custom/reset_student_scorm.php?reset=course&user=" . $user->id . "&course=" . $course->id . "\">Reset Progress in Whole Course (this cannot be reversed!)</a></div></div>";
