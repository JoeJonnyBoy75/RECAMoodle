...
trait separator_form {
    ...
    public function separator_style($formdata) {
        ...
        switch ($formdata['style']) {
            case 'blur':
                ...
                break;
            case 'blurend':
                $style .= "border: none; height: {$formdata['height']}px; background-image: linear-gradient(to right, transparent, {$formdata['color']}, transparent);";
                break;
            case 'gradient':
                $style .= "height: {$formdata['height']}px;
                border: none;
                background: linear-gradient(to right, {$formdata['color']} 0%, {$formdata['color2']} 50%, {$formdata['color']} 100%);";
                break;
            default:
    ...
    private function separatorform(&$mform, $formdata, $configdata) {
        ...
        $this->add_common_section_settings($mform, $formdata['sectionproperties'], $configdata['sectionproperties'], SEC_SEPARATOR);
        ...
        $mform->addElement('select', $title, get_string('separatorstyle', COMPONENT), $options, array('class' => 'ml-0 mr-5 mb-10 separator-style' ));
        ...
        $mform->addElement('select', $title, get_string('separator'.$title, COMPONENT), $options, array('class' => ' ml-0 mr-5 mb-10 separator-style' ));
        $mform->setDefault($title, $defaultval);
        ...
        $mform->addElement('select', $title, get_string('separatorheight', COMPONENT), $heights, array('class' => 'ml-0 mr-5 mb-10 separator-style' ));
        $mform->setDefault($title, $defaultval);
