...
trait team_form {
    ...
    private function teamform(&$mform, $formdata, $dbformdata) {
        ...
        $formdata[$title] = (!isset($formdata[$title])) ? $rows['text-center'] : $formdata[$title];
        ...
        for ($nrow = 0; $nrow < $formdata['rows']; $nrow++) {
            ...
            $defaultval = (!isset($dbformdata['row'][$nrow]['members'])) ? $rowscount['1'] : $dbformdata['row'][$nrow]['members'];
            $defaultval = (!isset($formdata['row'][$nrow]['members'])) ? $defaultval : $formdata['row'][$nrow]['members'];
            $title = 'row'.'_'.$nrow.'_members';
            $mform->addElement('select', $title, get_string('members', COMPONENT), $rowscount, array('class' => 'updateform ml-0 mr-5 mb-10 '));
            $mform->setDefault($title, $defaultval);
            ...
            for ($nmember = 0; $nmember < $formdata['row'][$nrow]['members']; $nmember++) {
                ...
                $defaultval = isset($dbformdata['row'][$nrow]['member'][$nmember]['name']) ? $dbformdata['row'][$nrow]['member'][$nmember]['name'] : [];
                $defaultval = isset($formdata['row'][$nrow]['member'][$nmember]['name']) ? $formdata['row'][$nrow]['member'][$nmember]['name'] : $defaultval;
                ...
                $formitemid = isset($formdata['row'][$nrow]['member'][$nmember]['itemid']) ? $formdata['row'][$nrow]['member'][$nmember]['itemid'] : null;
                $dbitemid = isset($dbformdata['row'][$nrow]['member'][$nmember]['itemid']) ? $dbformdata['row'][$nrow]['member'][$nmember]['itemid'] : null;
                ...
                $defaultval = isset($dbformdata['row'][$nrow]['member'][$nmember]['quote']) ? $dbformdata['row'][$nrow]['member'][$nmember]['quote'] : [];
                $defaultval = isset($formdata['row'][$nrow]['member'][$nmember]['quote']) ? $formdata['row'][$nrow]['member'][$nmember]['quote'] : $defaultval;
                $this->add_section_title_settings($mform, $defaultval, $textobj, $title);
            }
        }
    }
        ...
        foreach ($configdata['row'] as $krow => $vrow) {
            foreach ($configdata['row'][$krow]['member'] as $kmember => $member) {
                ...
                if ($itemid != "" || $itemid != null) {
                    if ($delete) {
                        $fs->delete_area_files($systemcontext->id, COMPONENT, SEC_TEAM, $itemid);
                    } else {
                        $newitemid = theme_remui_get_unused_itemid(SEC_TEAM);
                        file_save_draft_area_files($itemid, $systemcontext->id, COMPONENT, SEC_TEAM, $newitemid);
                        $configdata['row'][$krow]['member'][$kmember]['itemid'] = $newitemid;
                        $imgurl = get_file_img_url($newitemid, COMPONENT, SEC_TEAM);
                        if ($imgurl == "") {
    ...
    private function team_process_section_creation($id, $configdata) {
        ...
        foreach ($configdata['row'] as $rownum => $row) {
            foreach ($row['member'] as $memnum => $member) {
                ...
                $filepath = $CFG->dirroot . '/theme/remui/pix/' . $img;
