...
trait feature_form {
    ...
    private function featureform(&$mform, $formdata, $configdata) {
        ...
        $this->add_common_section_settings($mform, $formdata['sectionproperties'], $configdata['sectionproperties'], SEC_FEATURE, isset($formdata['fromform']));
        ...
        for ($nrow = 0; $nrow < $formdata['rows']; $nrow++) {
            ...
            $title = 'row'.'_'.$nrow.'_features';
            $mform->addElement('select', $title, get_string('features', COMPONENT), $rowscount, array('class' => 'updateform ml-0 mr-5 mb-10 '));
            ...
            for ($nfeature = 0; $nfeature < $featurecount; $nfeature++) {
                ...
                $defaultval = isset($configdata['row'][$nrow]['feature'][$nfeature]['name']) ? $configdata['row'][$nrow]['feature'][$nfeature]['name'] : [];
                $defaultval = isset($formdata['row'][$nrow]['feature'][$nfeature]['name']) ? $formdata['row'][$nrow]['feature'][$nfeature]['name'] : $defaultval;
                ...
                $defaultval = isset($configdata['row'][$nrow]['feature'][$nfeature]['icon']) ? $configdata['row'][$nrow]['feature'][$nfeature]['icon'] : [];
                $defaultval = isset($formdata['row'][$nrow]['feature'][$nfeature]['icon']) ? $formdata['row'][$nrow]['feature'][$nfeature]['icon'] : $defaultval;
                $this->add_section_title_settings($mform, $defaultval, $textobj, $title);
                ...
                $defaultval = isset($configdata['row'][$nrow]['feature'][$nfeature]['description']) ? $configdata['row'][$nrow]['feature'][$nfeature]['description'] : [];
                $defaultval = isset($formdata['row'][$nrow]['feature'][$nfeature]['description']) ? $formdata['row'][$nrow]['feature'][$nfeature]['description'] : $defaultval;
                $this->add_section_title_settings($mform, $defaultval, $textobj, $title);
