...
trait aboutus_form {
    ...
    private function aboutusform(&$mform, $formdata, $configdata) {
        ...
        $this->add_common_section_settings($mform, $formdata['sectionproperties'], $configdata['sectionproperties'], SEC_ABOUTUS);
        ...
        for ($blrow = 0; $blrow < 4; $blrow++) {
            ...
            $defaultval = (!isset($formdata['block'][$blrow]['blockbackground'])) ? 0 : $formdata['block'][$blrow]['blockbackground'];
            $mform->addElement('select', $title, get_string('blockbackground', COMPONENT), $options, array('class' => ' ml-0 mr-5 mb-10' ));
            $mform->setDefault($title, $defaultval);
    ...
    public function update_aboutus_file_area($configdata, $delete = true) {
        ...
        if (isset($configdata['image'])) {
            ...
            if ($itemid != "" || $itemid != null) {
                if ($delete) {
                    $fs->delete_area_files($systemcontext->id, COMPONENT, SEC_ABOUTUS, $itemid);
                } else {
                    $newitemid = theme_remui_get_unused_itemid(SEC_ABOUTUS);
                    file_save_draft_area_files($itemid, $systemcontext->id, COMPONENT, SEC_ABOUTUS, $newitemid);
                    $configdata['imageurl'] = get_file_img_url($newitemid, COMPONENT, SEC_ABOUTUS);
        ...
        for ($blrow = 0; $blrow < 4; $blrow++) {
            ...
            if ($itemid != "" || $itemid != null) {
                ...
                $newitemid = theme_remui_get_unused_itemid(SEC_ABOUTUS);
                file_save_draft_area_files($itemid, $systemcontext->id, COMPONENT, SEC_ABOUTUS, $newitemid);
                $configdata['block'][$blrow]['imageurl'] = get_file_img_url($newitemid, COMPONENT, SEC_ABOUTUS);
    ...
    /**
     * Create aboutus image before saving first instance.
     * @param  int    $id         instance id
     * @param  array  $configdata Default section data
     * @return array              updated form data
     */
    private function aboutus_process_section_creation($id, $configdata) {
        global $CFG, $USER;
        $fs = get_file_storage();
        $img = $configdata['imageurl'];
