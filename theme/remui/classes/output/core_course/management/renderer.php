...
class renderer extends \core_course_management_renderer {
    ...
    public function view_mode_selector(array $modes, $currentmode, moodle_url $url = null, $param = 'view') {
        ...
        $html = html_writer::start_div('view-mode-selector vms d-content');
    ...
    public function grid_column_start($size, $id = null, $class = null) {
        ...
        if ($id == 'course-detail') {
            ...
            $bootstrapclass = 'd-flex flex-wrap px-3 mb-3 m-10';
        }
    ...
    public function course_detail(core_course_list_element $course) {
        ...
        $html .= html_writer::start_div('px-0');
    ...
    public function category_listing(core_course_category $category = null) {
        ...
        $html  = html_writer::start_div('category-listing card w-p100');
        $html .= html_writer::tag('h3', get_string('categories'),
            array('class' => 'px-0 text-center', 'id' => 'category-listing-title'));
        $html .= html_writer::start_div('card-body px-0');
    ...
    public function category_listitem(core_course_category $category, array $subcategories, $totalsubcategories,
        ...
        $attributes = array(
            'class' => 'listitem listitem-category list-group-item list-group-item-action p-5',
            'data-id' => $category->id,
        ...
        $html .= html_writer::start_div('float-left ba-checkbox mr-0');
    ...
    public function category_listitem_actions(core_course_category $category, array $actions = null) {
        ...
        $menu = new action_menu();
        $menu->attributes['class'] .= ' category-item-actions item-actions mr-0';
        $hasitems = false;
    ...
    public function category_listing_actions(core_course_category $category = null) {
        ...
        if ($cancreatecategory) {
            ...
            $actions[] = html_writer::link($url, get_string('createnewcategory'), array('class' => ''));
        }
        ...
        return html_writer::div(join(' ', $actions), 'listing-actions category-listing-actions mb-10 p-5');
    }
    ...
    public function course_listing(core_course_category $category = null, core_course_list_element $course = null,
        ...
        $html  = html_writer::start_div('card course-listing w-p100'.$class, array(
        ...
        $html .= html_writer::tag('h3', $category->get_formatted_name(),
            array('id' => 'course-listing-title', 'tabindex' => '0', 'class' => 'px-0 text-center'));
        $html .= html_writer::start_div('card-body px-0');
        $html .= $this->course_listing_actions($category, $course, $perpage);
        $html .= $this->listing_pagination($category, $page, $perpage, false, $viewmode);
        $html .= html_writer::start_tag('ul', array('class' => 'ml course-list m-0 p-0', 'role' => 'group'));
    ...
    public function course_listitem(core_course_category $category, core_course_list_element $course, $selectedcourse) {
        ...
        $attributes = array(
            'class' => 'listitem listitem-course list-group-item list-group-item-action p-10',
            'data-id' => $course->id,
    ...
    public function course_listing_actions(core_course_category $category, core_course_list_element $course = null, $perpage = 20) {
        ...
        if ($category->can_create_course()) {
            ...
            $actions[] = html_writer::link($url, get_string('createnewcourse'), array('class' => ''));
        }
        ...
        $actions[] = $this->render($menu);
        return html_writer::div(join(' ', $actions), 'listing-actions course-listing-actions mb-10');
    }
    ...
    public function category_bulk_actions(core_course_category $category = null) {
        ...
        if (core_course_category::can_resort_any()) {
            ...
            $form .= html_writer::div(
                html_writer::select(
                    ...
                    'selectedcategories',
                    false,
                    array('aria-label' => get_string('selectcategorysort'), 'class' => 'w-full')
                )
            );
            $form .= html_writer::div(
                html_writer::select(
                    ...
                    'name',
                    false,
                    array('aria-label' => get_string('selectcategorysortby'), 'class' => 'm-t-1 w-full')
                )
            );
            $form .= html_writer::div(
                html_writer::select(
                    ...
                    'fullname',
                    false,
                    array('aria-label' => get_string('selectcoursesortby'), 'class' => 'm-t-1 w-full')
                )
            );
            ...
            $html .= html_writer::div(html_writer::span(get_string('sorting')), 'pair-key span3 col-md-12 yui3-u-1-4');
            $html .= html_writer::div($form, 'pair-value span9 col-md-12 yui3-u-3-4');
            $html .= html_writer::end_div();
        }
        if (core_course_category::can_change_parent_any()) {
            ...
            $select = html_writer::select(
                ...
                '',
                array('' => 'choosedots'),
                array('aria-labelledby' => 'moveselectedcategoriesto', 'class' => 'm-r-1 w-full mb-5')
            );
    ...
    public function course_bulk_actions(core_course_category $category) {
        ...
        if ($category->can_move_courses_out_of()) {
            ...
            $select = html_writer::select(
                ...
                '',
                array('' => 'choosedots'),
                array('aria-labelledby' => 'moveselectedcoursesto', 'class' => 'm-r-1 w-full mb-5')
            );
    ...
    protected function detail_pair($key, $value, $class ='') {
        ...
        $colleft = 'pair-key span3 yui3-u-1-4 '.$lcol;
        $colright = 'pair-value span9 m-b-1 yui3-u-3-4 form-inline '.$rcol;

        $html = html_writer::start_div('detail-pair row yui3-g '.preg_replace('#[^a-zA-Z0-9_\-]#', '-', $class));
        $html .= html_writer::div(html_writer::span($key), $colleft);
        $html .= html_writer::div(html_writer::span($value), $colright);
        $html .= html_writer::end_div();
        return $html;
    }
    /**
    ...
    public function course_detail_actions(core_course_list_element $course) {
        ...
        return html_writer::div(join('|', $options), 'listing-actions course-detail-listing-actions p-10');
    }
