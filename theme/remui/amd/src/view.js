...
import $ from 'jquery';
import * as Repository from 'theme_remui/repository';
...
const renderCourses = (root, coursesData) => {
    ...
    if (!coursesData) {
        ...
        coursesData.courses = coursesData.courses.map(course => {
            ...
            coursedate.day = date.getDate();
            coursedate.month = months[date.getMonth()];
            coursedate.year = date.getFullYear();
        ...
        if (coursesData.courses.length) {
            return Templates.render(currentTemplate, {
                ...
                carddesignsetting: (coursecarddesignsettingdata == 0) ? true : false,
                layoutprogressbarstyle: (coursecarddesignsettingdata == 0) ? 'layout1-progressbar' : 'layout2-progressbar',
                cardanimation: coursecardanimationsetting,
                courseprogresstext: true
            });
        } else {
            return noCoursesRender(root);
        }
...
export const clearSearch = (clearIcon, root) => {
    ...
    init(root, coursecarddesignsettingdata, coursecardanimationsetting);
};
...
var coursecarddesignsettingdata = '';
var coursecardanimationsetting = '';
/**
 * Intialise the courses list and cards views on page load.
 *
 * @param {object} root The root element for the courses view.
 * @param {object} coursecarddesignsetting The root element for the courses view.
 * @param {object} cardanimationsetting The root element for the courses view.
 */
export const init = (root, coursecarddesignsetting, cardanimationsetting) => {
    ...
    lastPage = 0;
    courseOffset = 0;
    coursecarddesignsettingdata = coursecarddesignsetting;
    coursecardanimationsetting = cardanimationsetting;
