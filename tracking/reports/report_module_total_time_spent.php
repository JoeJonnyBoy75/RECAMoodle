...
foreach ($user_rows as $user) {
    $tmp_sessions = get_user_session_summaries($cm, $user->user_id, $epoch_from, $epoch_to);
    $tm = 0;
    $attempts = 0;
    $top_score = -1;
    foreach ($tmp_sessions as $tmp_session) {
        $tm += ($tmp_session['end'] - $tmp_session['start']);
        $attempts += $tmp_session['num_attempts'];
        if ($top_score < $tmp_session['top_score']) {
            $top_score = $tmp_session['top_score'];
        }
    }
    $users[] = array('user'=>$user, 'num_entries'=>count($tmp_sessions), 'total_time'=>$tm / 1000., 'num_attempts'=>$attempts, 'top_score'=>$top_score > -1 ? $top_score : '');
    $total_times[] = $tm / 1000.;
}
...
<body>
	<h1>Total Time For Users in Course Module</h1>
	<table>
		<tbody>
			<tr>
				<th>Course</th>
		...
		<tbody>
			...
			<tr>
				...
				<td><?php echo $user['user']->username; ?></td>
                                ...
                                <td><?php echo $user['num_attempts']; ?></td>
